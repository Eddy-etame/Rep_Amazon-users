...
import type { DeliveryAddress } from './depot-carnet-adresses';
import type { CartItem } from './depot-panier';
import type { CatalogProduct } from './depot-catalogue-produits';
import { parseLifecycleStatus, type OrderLifecycleStatus } from '../utils/statut-commande';
import { ServiceIA } from './service-ia';
import { DepotCatalogueProduits } from './depot-catalogue-produits';
import { ServiceCommandes } from './service-commandes';
...
/** Transitions de statut (append-only) depuis order-service (`statusHistory`). */
...
export interface CommandeState {
  id: string;
  /** Statut de traitement depuis l'API (1:1 avec order-service). */
  lifecycleStatus: OrderLifecycleStatus;
  createdAt: number;
  updatedAt?: number;
  /** Minutes depuis la création de la commande (informatif, pas l'état d'expédition). */
  t: number;
  validite: Validite;
  items?: CommandeItem[];
  total?: number;
  ...
  /** Historique d'audit (le plus récent en premier) quand l'API le renvoie. */
...
/** Rotation parmi les produits en stock pour que les cartes accueil changent sans requête HTTP supplémentaire. */
...
export class DepotEtatCommandes {
  private readonly ordersSignal = signal<CommandeState[]>([]);
  private readonly receiptByOrderId = new Map<string, string>();

  /** Identifiants produit depuis POST /ai/recommendations (résolus dans le catalogue). */
  private aiBoostIds: string[] = [];
  private lastAiFetchMs = 0;
  /** Quand la requête dérivée des commandes change, relancer même dans la fenêtre de throttle. */
  private lastAiQueryFingerprint: string | null = null;
  private aiRefreshInFlight = false;

  /** Vrai tant que GET /commandes est en cours. */
  ...
  constructor(
    ...
    effect(() => {
      ...
      if (!session) {
        ...
        this.lastAiQueryFingerprint = null;
  ...
  /** Construit une courte requête texte pour le recommandeur démo à partir des titres de lignes de commandes récentes. */
  private buildRecommendationQueryFromOrders(): string {
    const orders = this.ordersSignal();
    const seen = new Set<string>();
    const parts: string[] = [];
    outer: for (const o of orders.slice(0, 10)) {
      for (const it of o.items || []) {
        const t = String(it.titre || '').trim();
        if (t.length < 3) continue;
        const key = t.toLowerCase();
        if (seen.has(key)) continue;
        seen.add(key);
        parts.push(t.split(/\s+/).slice(0, 5).join(' '));
        if (parts.length >= 8) break outer;
      }
    }
  ...
  private async refreshAiRecommendations(): Promise<void> {
    ...
    const fingerprint = this.buildRecommendationQueryFromOrders().trim();
    if (
      this.lastAiFetchMs > 0 &&
      now - this.lastAiFetchMs < 300_000 &&
      fingerprint === this.lastAiQueryFingerprint
    ) {
      return;
    }
    ...
    try {
      ...
      this.lastAiQueryFingerprint = fingerprint;
  ...
  /** Vrai quand updatedAt de l'API est plus récent que le dernier acquittement (sessionStorage). */
  ...
  /** Fusionne ou ajoute une commande (ex. après GET /commandes/:id). */
  ...
  storeReceipt(orderId: string, receiptDataUrl: string): void {
    this.receiptByOrderId.set(orderId, receiptDataUrl);
  }
