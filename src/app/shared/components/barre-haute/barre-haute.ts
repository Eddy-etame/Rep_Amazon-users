...
import { ServiceProduits } from '../../../core/services/service-produits';
import { DepotSessionUtilisateur } from '../../../core/services/depot-session-utilisateur';
import { PipeDeviseAmaz } from '../../pipes/pipe-devise';
...
@Component({
  ...
  imports: [CommonModule, FormsModule, RouterLink, RouterLinkActive, PipeDeviseAmaz],
  templateUrl: './barre-haute.html',
...
export class BarreHaute implements OnInit {
  ...
  readonly popularSearches = ['Casque', 'Smartphone', 'Cuisine', 'Ordinateur', 'Mode'];
  ...
  constructor(
    ...
    private readonly productsApi: ServiceProduits,
  ...
  /** Categories for the top-bar row: live from `DepotCatalogueProduits`, sorted (signal-safe). */
  ...
  private async runSuggestionUpdate(): Promise<void> {
    ...
    this.buildKeywordSuggestions(qRaw);
  ...
  onDocumentKeydown(event: KeyboardEvent): void {
    ...
    const nav = this.hostRef.nativeElement.querySelector('#main-nav');
    if (!nav) {
      return;
    }
    ...
    const visibleFocusables = focusables.filter((el) => el.offsetParent !== null) as HTMLElement[];
    if (!visibleFocusables.length) {
      return;
    }
    const first = visibleFocusables[0] as HTMLElement;
    const last = visibleFocusables[visibleFocusables.length - 1] as HTMLElement;
