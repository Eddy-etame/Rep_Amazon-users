...
import { ServiceRetourCommande, type OrderReturnRequest } from '../../core/services/service-retour-commande';
import { mapOrderFromApi, DepotEtatCommandes } from '../../core/services/depot-etat-commandes';
import { ServiceCommandes } from '../../core/services/service-commandes';
...
} from '../../core/utils/statut-commande';
...
@Component({
  ...
  imports: [RouterLink, FormsModule, PipeDeviseAmaz, PipeLibelleStatutCommande],
  templateUrl: './detail-commande.html',
  styleUrl: './detail-commande.scss'
})
export class DetailCommande implements OnInit, OnDestroy {
  ...
  constructor(
    ...
    readonly orderReturn: ServiceRetourCommande,
  ...
  get returnQrImageUrl(): string {
    const req = this.returnRequest;
    return req ? this.orderReturn.qrImageUrl(req) : '';
  }
  ...
  async ngOnInit(): Promise<void> {
    void this.refreshOrder();
    void this.orderReturn.loadForUser().then(() => this.cdr.detectChanges());
  ...
  async requestReturn(): Promise<void> {
    ...
    const items = itemIds.map((productId) => {
      const lineItem = o.items?.find((i) => i.productId === productId);
      return { productId, quantity: lineItem?.quantite ?? 1 };
    });

    this.returnLoading = true;
    this.returnError = '';
    try {
      await this.orderReturn.createReturnRequest({
        orderId: id,
        items,
        reason
      });
      this.returnFormOpen = false;
      this.returnReason = '';
      this.returnSuccess = 'Demande de retour enregistrée. Un QR code a été généré.';
    } catch (err: unknown) {
      const msg = err instanceof Error ? err.message : '';
      this.returnError = msg || 'Impossible de créer la demande de retour. Réessayez.';
    } finally {
  ...
  async downloadReturnQr(): Promise<void> {
    ...
    if (!request) return;
    const qrUrl = this.orderReturn.qrImageUrl(request);
    ...
    try {
      ...
      link.href = objectUrl;
      link.download = `retour-${request.id}.png`;
      link.click();
